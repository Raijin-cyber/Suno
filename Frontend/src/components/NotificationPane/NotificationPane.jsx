...
const NotificationPane = ({ conversationRefresh, isNotifiOpen=false, setIsNotifiOpen, notifications=[] }) => {
    return (
        <div id="notification pane" className={`${isNotifiOpen ? "translate-x-0" : "-translate-x-full"} z-40 absolute left-0 top-0 p-3 flex flex-col gap-y-5 ease-in-out max-md:bottom-0 overflow-hidden md:left-0 md:top-0 transition-all duration-500 bg-[#fc94Af] h-full w-full max-w-screen`}>
            ...
            <div className="flex flex-col gap-y-2 h-full w-full py-5 px-4 rounded-3xl shadow-[inset_6px_6px_5px_#de829a,inset_-6px_-6px_5px_#ffa6c4]">
                {
                    notifications?.map((notifi) => {
                        if(notifi.type === "request") return <NotificationSnippet key={notifi._id} notificationContent={notifi} conversationRefresh={conversationRefresh} requestNotification={true} />
